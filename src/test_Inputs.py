...
class InputTest(unittest.TestCase):
    ...
    def test_invalid_Joint_count(self):
        ...
        f = open(filename, "r")
        inParams = Inputs.getfile(f)
        err=Inputs.verify(inParams[0],inParams[1],inParams[2],inParams[3],inParams[4],inParams[5],inParams[6])
        f.close()
        assert not(err), "Something wrong in the verify module Inputs.py"
